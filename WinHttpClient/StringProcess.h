...
inline std::time_t getEPOCFromResponseHeaders(std::wstring responseHeaders)
{
	std::tstring delimiter = TXET(_T("\r\n"));

	size_t pos = 0;
	std::wstring token;
	std::wstring date;
	while ((pos = responseHeaders.find(delimiter)) != std::wstring::npos)
	{
		token = responseHeaders.substr(0, pos);
		if ( token.find(L"Date:") == 0 ) {
		date = token.substr(6,token.length()-6 - 4);
		break;
		}
		responseHeaders.erase(0, pos + delimiter.length());
	}
	
	// Sun, 05 Apr 2015 12:02:35 GMT
	struct std::tm tm_epoch;
	std::istringstream ss(ws2s(date));
	ss >> std::get_time(&tm_epoch, "%a, %d %b %Y %H:%M:%S");
	std::time_t time_epoch_in_sec = mktime(&tm_epoch);
	return time_epoch_in_sec;  
}
